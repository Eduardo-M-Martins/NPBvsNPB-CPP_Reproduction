...
void write_ft_info(FILE *fp, char class) 
{
  ...
  fprintf(fp, "%sinteger nx, ny, nz, maxdim, niter_default, dflt\n", FINDENT);
...
setparams: File %s doesn't exist. To build the NAS benchmarks\n\
  ...
  char tmp2[10];
  sprintf(tmp2, "%d.%d.%d", __GNUC__,__GNUC_MINOR__,__GNUC_PATCHLEVEL__);
  char tmp3[10];
  ...
  switch(type) {
      ...
      case UA:
          ...
          put_string(fp, "libver", tmp3);
          put_string(fp, "cver", tmp2);
      ...
      case DC:
          ...
          put_def_string(fp, "LIBVER", tmp3);
          put_def_string(fp, "CVER", tmp2);
